...
Wayland 支持 WebUI 模式，允许通过浏览器访问应用。这对于远程使用 OpenClaw 非常有用。Wayland 提供三种远程连接方式，满足不同场景的需求。

**重要**：WebUI 配置应通过 Wayland 设置界面完成，无需使用命令行。本指南将引导你如何在设置界面中完成配置。
...
- 在 Wayland 主界面，使用快捷键打开设置（具体快捷键请查看 Wayland 帮助文档）
...
1. 确保远程设备与 Wayland 电脑在同一 WiFi 网络
...
#### Step 1: 在 Wayland 电脑上配置 WebUI

1. **打开 WebUI 设置界面**：
   - 在 Wayland 主界面，点击左下角的**设置图标**
   - 点击 **"WebUI"** 选项
...
#### Step 2: 在 Wayland 电脑上安装并登录 Tailscale
...
1. 在 Wayland 电脑上，打开 Tailscale 应用
...
#### Step 1: 在服务器上安装 Wayland

按照 Wayland 安装指南在服务器上安装 Wayland 应用。

#### Step 2: 通过设置界面配置 WebUI

1. **打开 WebUI 设置界面**：
   - 如果服务器有图形界面，直接打开 Wayland 应用
...
如果需要 Wayland 开机自启，可以配置 systemd 服务。但建议通过 Wayland 设置界面管理 WebUI，而不是通过命令行。
...
按照 Wayland 安装指南在 macOS 服务器上安装 Wayland 应用。
...
启动 WebUI 后，可以通过浏览器访问 Wayland，然后：

1. **在首页找到 OpenClaw 入口**（ACP 代理列表）
2. **直接与 OpenClaw 对话**
3. **享受完整的 Wayland 界面功能**：
   - 文件预览和管理
   - 多对话管理
   - 完整的工具和技能支持
...
1. **打开设置**：点击 Wayland 左下角的**设置图标** → 点击 **"WebUI"**
