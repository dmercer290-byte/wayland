...
Wayland 支持 CDP 用于外部调试工具集成。在开发模式 (`just dev`) 下，CDP 默认在端口 9230 启用。

## 在生产环境启用 CDP

1. 打开 Wayland 设置 → 系统 → 开发者调试
2. 启用"启用远程调试 (CDP)"
3. 重启应用
...
配置完成后，可以使用 MCP 工具与 Wayland 交互：

- `list_pages` — 列出 Wayland 中所有打开的页面
