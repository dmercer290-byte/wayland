...
Wayland 设置页 > Agent > Local Agent 页，APP 启动时自动扫描本地已安装的 CLI backend（Claude Code、Opencode、Kimi 等），将检测到的 agent 展示在列表中供用户使用。
...
3. **不知道有哪些可用**：用户不知道 Wayland 支持哪些 agent backend，缺少发现入口
...
| **远程** | 从 GitHub `wayland/hub` 仓库拉取的 index.json 中，存在于远程但不在内置列表中的 extension | 需要网络 |
...
由 CI 从 `TradeCanyon/WaylandHub` 仓库自动生成，客户端拉取此文件获取可用 extension 列表。
...
```json
{
  ...
  "extensions": {
    "aionext-auggie": {
      ...
      "version": "1.0.0",
      "description": "Integrates Augment Code as an ACP adapter in Wayland.",
      "author": "Wayland Official",
      "engines": {
        "wayland": "^1.0.0"
      },
      "hubs": ["acpAdapters"],
    ...
    "aionext-claude": {
      ...
      "version": "1.0.0",
      "description": "Integrates Anthropic's Claude Code CLI as an ACP adapter in Wayland.",
      "author": "Wayland Official",
      "engines": {
        "wayland": "^1.0.0"
      },
...
`engines.wayland` 定义了兼容的 Wayland 版本范围，
...
判断 Hub extension 的展示状态:
  ...
  extension 目录不存在 (~/.wayland/extensions/aionext-xxx/)
...
| 状态信号           | 数据来源                                         | 说明           |
| ------------------ | ------------------------------------------------ | -------------- |
| extension 是否存在 | `ExtensionLoader` 扫描 `~/.wayland/extensions/`   | 已有能力       |
...
APP 启动
  │
  ├── 1. ExtensionLoader 扫描 ~/.wayland/extensions/ (现有逻辑)
  │      → 得到已安装 extension 列表
  │
...
用户点击 Install
  ...
  │
  ├── 4. 解压到临时目录
  │      └── ~/.wayland/extensions/.tmp/aionext-xxx/
  │
  ├── 5. 校验 aion-extension.json 合法性
  │      └── 失败 → 清理临时目录, 状态 → install_failed
  │
  ├── 6. 原子移动到正式目录
  │      └── .tmp/aionext-xxx/ → ~/.wayland/extensions/aionext-xxx/
  │
...
用户点击 Uninstall (更多菜单中)
  ...
  │      └── 失败 → 仅 warn 日志, 不阻塞卸载
  │
  ├── 5. 删除 ~/.wayland/extensions/aionext-xxx/ 目录
  │
...
用户点击 Retry (在 install_failed 状态)
  │
  ├── 检查 ~/.wayland/extensions/aionext-xxx/ 是否已存在
...
### 6.1 GitHub 仓库 (WaylandHub)

> (拆另外仓库, 比如 `TradeCanyon/WaylandHub`，专门存放 Hub 相关的 extension 和构建脚本)
...
```
Wayland.app/Contents/Resources/
└── hub/
    ├── index.json                   ← 构建 APP 时从 wayland/hub 的 dist/ 复制
...
拉取 remote-index.json
  ├── 优先: GitHub Raw
  │   https://raw.githubusercontent.com/wayland/hub/main/dist/index.json
  │
  ├── 备选: jsDelivr CDN
  │   https://cdn.jsdelivr.net/gh/wayland/hub@main/dist/index.json
  │
  ├── 超时: 5s (不阻塞 APP 启动)
  ├── 重试: 1 次
  │
...
for (const hubAgent of hubAgents) {
  ...
  const extDir = `~/.wayland/extensions/${hubAgent.name}/`;
