...
use serde_json::json;
use wcore_agent::compact::auto::{BOUNDARY_PREFIX, autocompact, should_autocompact};
use wcore_agent::compact::emergency::is_at_emergency_limit;
use wcore_agent::compact::micro::{CLEARED_TOOL_RESULT, microcompact};
...
///
/// M2.6: Gated behind the `live-openai` cargo feature. Default CI does not
/// compile or run this test. Enable manually with:
///   cargo nextest run -p wcore-agent --features live-openai --test acceptance \
///       -E 'test(autocompact_triggers_llm_summary)'
...
async fn autocompact_triggers_llm_summary() {
    let api_key = helpers::openai_api_key().expect(
        "[acceptance] OPENAI_API_KEY required when --features live-openai is enabled \
    ...
    let config = {
        ...
        wcore_config::config::Config {
            model: crate::common::models::openai_gpt4_1_mini(),
            ..base
        }
    };
