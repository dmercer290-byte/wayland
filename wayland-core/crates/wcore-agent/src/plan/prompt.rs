...
/// A standing preference injected into the base system prompt when the user
/// has enabled `[plan] plan_first` (via `/config`). Unlike
/// [`plan_mode_instructions`] (only present *while* plan mode is active), this
/// nudges the agent to *enter* plan mode on its own for risky work. It is a
/// preference, not a hard gate — small, low-risk edits proceed directly.
pub fn plan_first_instruction() -> &'static str {
    "# Plan-first preference\n\n\
     The user prefers a plan before large or risky changes. When a request \
     would touch many files, perform a destructive or irreversible operation, \
     or carry meaningful risk, FIRST call the EnterPlanMode tool to present a \
     short plan and wait for approval before editing. For small, low-risk, or \
     read-only work, proceed directly — do not over-plan trivial changes."
}
...
mod tests {
    ...
    #[test]
    fn plan_first_instruction_nudges_planning_without_hard_gating() {
        let text = plan_first_instruction();
        // It must steer the agent to the EnterPlanMode tool for risky work…
        assert!(text.contains("EnterPlanMode"), "must name the plan tool");
        // …but stay a preference, not a hard gate — trivial work proceeds.
        assert!(
            text.contains("proceed directly"),
            "must allow small/low-risk work to skip planning"
        );
    }
