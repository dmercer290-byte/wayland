...
fn block_regex() -> &'static Regex {
    ...
    // SAFETY: static literal regex; unit tests below exercise this
    // pattern so any breakage in the literal is caught before release.
...
fn inline_line_start_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    // SAFETY: static literal regex; verified by the unit tests below.
...
fn extract_shell_matches(content: &str) -> Vec<ShellMatch> {
    ...
    for cap in block_regex().captures_iter(content) {
        // SAFETY: cap.get(0) is always Some — it's the full match
        // produced by captures_iter, by `regex` crate contract.
    ...
    for cap in inline_line_start_regex().captures_iter(content) {
        // SAFETY: groups 1 and 2 are both `(...)` in the pattern; a
    ...
    for cap in inline_whitespace_regex().captures_iter(content) {
        // SAFETY: groups 2 and 3 are both `(...)` in the pattern; a
        // successful match always populates them.
