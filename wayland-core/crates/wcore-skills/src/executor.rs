...
/// argument + env-var substitution — WITHOUT running any embedded shell
/// commands.
///
/// This is the single source of truth for the post-substitution body. Both
/// [`prepare_inline_content`] (which then shell-executes the result) and the
/// `wcore-agent` cron skill-dispatch sink (which scans the result with the
/// `wcore-cron` execution-boundary denylist BEFORE dispatch) call this, so the
/// bytes that get scanned are byte-identical to the bytes that reach `sh -c`.
/// Splicing the cron `args` into a `!shell:` body line cannot evade the scan
/// because the scan runs on this composed string, not on the raw body + raw
/// args separately.
pub fn render_shell_input(
    skill: &SkillMetadata,
    args: Option<&str>,
...
///
/// Steps 1+2 are delegated to [`render_shell_input`] so the composed text is a
/// single source of truth shared with the cron skill-dispatch sink's
/// pre-dispatch scan.
///
...
mod tests {
    ...
    async fn test_prepare_inline_substitutes_skill_dir() {
        let skill = make_skill("Dir: ${WCORE_SKILL_DIR}", Some("/skills/mine"));
        let result = prepare_inline_content(&skill, None, None, "/tmp")
            .await
            .unwrap();
    ...
    async fn test_prepare_inline_substitutes_session_id() {
        let skill = make_skill("Session: ${WCORE_SESSION_ID}", None);
...
mod supplemental_tests {
    ...
    // TC-10.4: Block shell command is executed and substituted
    #[tokio::test]
    ...
    // TC-10.5: Inline shell command is executed and substituted
    #[tokio::test]
    ...
    // TC-10.6: MCP skill skips shell - shell syntax in content is preserved verbatim
    #[tokio::test]
    ...
    // TC-10.7: variable substitution + shell ordering - variables are substituted first, then shell executes
    #[tokio::test]
    ...
    // TC-10.8: cwd argument is passed through to execute_shell_commands
    #[tokio::test]
