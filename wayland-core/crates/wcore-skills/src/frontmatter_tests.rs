...
fn tc_4_5_model_non_inherit_preserved() {
    let fm = FrontmatterData {
        model: Some(ANTHROPIC_OPUS.into()),
        ..Default::default()
    };
    let meta = parse_skill_fields(&fm, "", "x", SkillSource::User, LoadedFrom::Skills, None);
    assert_eq!(meta.model.as_deref(), Some(ANTHROPIC_OPUS));
}
...
#[test]
fn artifacts_field_parses_from_yaml_list() {
    ...
    assert_eq!(meta.artifacts[0].path, "report.md");
    assert!(meta.artifacts[0].template.contains("${args.target}"));
}

#[test]
fn artifacts_field_optional_defaults_empty() {
    let raw = "---\nname: x\ndescription: y\n---\n\nBody\n";
    let parsed = parse_frontmatter(raw);
    let meta = parse_skill_fields(
        &parsed.frontmatter,
