...
pub async fn load_mcp_skills(manager: &McpManager) -> Vec<LoadedSkill> {
    ...
    for server_name in manager.server_names() {
        ...
        for resource in resources {
            ...
            let source_label = format!("mcp:{server_name}:{}", resource.uri);
            let parsed = parse_frontmatter_with_source(&text, Some(&source_label));
...
mod tests {
    ...
    #[test]
    fn tc_wb_uri_simple() {
        // [white-box] skill://my-skill -> server:my-skill
        assert_eq!(
    ...
    fn tc_wb_uri_nested_one_slash() {
        // [white-box] TC-3.4: skill://db/migrate -> server:db:migrate
        assert_eq!(
    ...
    fn tc_wb_uri_nested_two_slashes() {
        // [white-box] TC-3.5: skill://a/b/c -> server:a:b:c
        assert_eq!(uri_to_skill_name("demo", "skill://a/b/c"), "demo:a:b:c");
    }

    #[test]
    fn tc_wb_uri_no_skill_prefix_passthrough() {
        // [white-box] strip_prefix returns uri unchanged when prefix not present;
    ...
    fn tc_wb_uri_empty_path_after_prefix() {
        // [white-box] skill:// -> server: (empty name part)
        assert_eq!(uri_to_skill_name("srv", "skill://"), "srv:");
    }

    // -----------------------------------------------------------------------
    // TC-3.x: load_mcp_skills [black-box + white-box]
    // -----------------------------------------------------------------------

    #[tokio::test]
    async fn tc_3_1_load_mcp_skills_normal() {
        // [black-box] TC-3.1: normal discovery - skill:// resource parsed to SkillMetadata
    ...
    async fn tc_3_2_uri_filter_skips_non_skill_uris() {
        // [black-box] TC-3.2: only skill:// URIs processed - tool:// and file:// are skipped
        let list_resp =
    ...
    async fn tc_3_3_naming_rule_simple() {
        // [black-box] TC-3.3: skill://my-skill -> demo:my-skill
        let list_resp = make_list_response(vec!["skill://my-skill"]);
        let read_resp = make_read_response("---\ndescription: x\n---\n");
    ...
    async fn tc_3_4_naming_rule_slash_to_colon() {
        // [black-box] TC-3.4: skill://db/migrate -> demo:db:migrate
    ...
    async fn tc_3_6_source_and_loaded_from_mcp() {
        // [black-box] TC-3.6/3.7: source=Mcp, loaded_from=Mcp, skill_root=None
    ...
    async fn tc_3_8_frontmatter_parsed() {
        // [black-box] TC-3.8: frontmatter fields properly parsed from MCP skill content
    ...
    async fn tc_3_9_single_resource_failure_does_not_affect_others() {
        // [black-box] TC-3.9: when read_resource fails for one skill, others still load
    ...
    async fn tc_3_10_server_list_failure_does_not_affect_other_servers() {
        // [black-box] TC-3.10: when list_resources fails for one server, other servers still load
    ...
    async fn tc_3_12_server_without_resources_capability_skipped() {
        // [black-box] TC-3.12: server without resources capability is not queried
        let manager = McpManager::new_for_test(vec![(
            "no-resources-server",
    ...
    async fn tc_3_13_multiple_servers_aggregated() {
        // [black-box] TC-3.13: skills from multiple servers all appear in results
    ...
    async fn tc_3_wb_virtual_path_format() {
        // [white-box] MCP skill virtual path is "<mcp:server:name>" for deduplication
        let list_resp = make_list_response(vec!["skill://my-skill"]);
        let read_resp = make_read_response("---\ndescription: x\n---\n");
