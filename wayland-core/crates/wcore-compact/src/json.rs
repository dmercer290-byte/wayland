...
/// Hard recursion cap for `format_value`. serde's parse-time recursion
/// limit (128) bounds nesting in practice, but if `disable_recursion_limit`
/// is ever set upstream a deeply-nested value could blow the stack here.
/// Beyond this depth we emit a truncation marker instead of recursing.
const MAX_DEPTH: usize = 128;

/// Emitted in place of any object/array nested deeper than `MAX_DEPTH`.
const TRUNCATION_MARKER: &str = "\"…(max depth)\"";
...
fn format_value(value: &serde_json::Value, depth: usize) -> String {
    if depth >= MAX_DEPTH {
        // Only containers recurse; scalars are always safe to render inline.
        if matches!(
            value,
...
mod tests {
    ...
    #[test]
    fn format_value_caps_recursion_depth() {
        // Build a value nested far deeper than MAX_DEPTH and confirm
        // format_value returns (no stack overflow) and inserts the
        // truncation marker rather than recursing the whole way down.
        //
        // Each level must be too wide to inline (> INLINE_THRESHOLD) or the
        // oneliner short-circuit would collapse the deep subtree at a shallow
        // depth and never reach the cap. A 100-char pad string at every
        // level guarantees descent all the way down.
        let pad = "x".repeat(100);
        let mut v = serde_json::json!("leaf");
        for _ in 0..(MAX_DEPTH + 50) {
            v = serde_json::json!({ "pad": pad, "child": v });
        }
        let out = format_value(&v, 0);
        assert!(
            out.contains(TRUNCATION_MARKER),
            "deeply nested value should be truncated, got: {out}"
        );
    }
}
