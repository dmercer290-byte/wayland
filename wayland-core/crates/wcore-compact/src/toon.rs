pub fn toon_encode_array(value: &serde_json::Value) -> Option<String> {
    ...
    for item in arr {
        // SAFETY: every element was confirmed to be an object with
        // exactly `fields` keys by the validation loop above; otherwise
        // the function returned `None` at that point.
        let obj = item.as_object().unwrap();
        result.push_str("  ");
        let values: Vec<String> = fields
            .iter()
            // SAFETY: `fields` was constructed from `first.keys()` and
            // every later `item` was checked to have the same key set.
