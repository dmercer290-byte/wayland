...
use wcore_tools::file_cache::FileStateCache;
...
async fn tc_5_3_06_nonexistent_file_error_no_cache() {
    ...
    // Use a real tempdir for a platform-agnostic absolute path. The
    // file inside is never created — we want the read to fail with
    // "Failed to read file", not the path-validation "NotAbsolute"
    // branch (Windows wants C:\..., not /tmp/..., CI run 25959637824).
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("does_not_exist_tc_5_3_06.txt");
