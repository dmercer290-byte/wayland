...
export default ({ config }: ConfigContext): ExpoConfig => {
  return {
    ...
    scheme: 'wayland-mobile',
    userInterfaceStyle: 'automatic',
    ios: {
      supportsTablet: true,
      bundleIdentifier: 'ai.resopod.wayland',
      buildNumber: String(VERSION.buildNumber),
      infoPlist: {
        ITSAppUsesNonExemptEncryption: false,
        NSCameraUsageDescription: 'Wayland needs camera access to scan QR codes for server login.',
      },
    },
    android: {
      adaptiveIcon: {
